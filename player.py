...
class Player(pygame.sprite.Sprite):
    ...
    def _setup_rects(self, pos):
        ...
        self.collision_rect = self.rect.inflate(
            -self.COLLISION_SHRINK, -self.COLLISION_SHRINK
        )
    ...
    def _move_axis(self, axis, dt):
        ...
        setattr(
            self.collision_rect,
            attr,
            getattr(self.collision_rect, attr)
            + getattr(self.direction, attr) * self.SPEED * dt,
        )
        ...
        for sprite in self.collision_sprites:
            if sprite.rect.colliderect(self.collision_rect):
                ...
                if direction_value > 0:  # Moving positive direction
                    setattr(
                        self.collision_rect, pos_side, getattr(sprite.rect, neg_side)
                    )
                elif direction_value < 0:  # Moving negative direction
                    setattr(
                        self.collision_rect, neg_side, getattr(sprite.rect, pos_side)
                    )
